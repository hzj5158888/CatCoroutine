...
private:
    ...
    public:
        ...
        explicit Node(const uint8_t height, const T & k, bool isHead, uint32_t cnt) : key(k), height(height), ref_count(cnt) { construct(isHead); }
        explicit Node(const uint8_t height, T && k, bool isHead, uint32_t cnt) : key(std::move(k)), height(height), ref_count(cnt) { construct(isHead); }
        ...
        static void destroy(Allocator & alloc, Node * node)
        {
            ...
            node->~Node();
            alloc.deallocate(reinterpret_cast<uint8_t*>(node), sizeof(Node) + sizeof(std::atomic<void*>) * height);
        }

        template<typename ... Args>
        static Node * create(Allocator & alloc, const uint8_t height, Args &&... args)
        {
            Node * ans = reinterpret_cast<Node*>(alloc.allocate(sizeof(Node) + sizeof(std::atomic<void*>) * height));
    ...
    [[nodiscard]] int randHeight(int max_h = MAX_HEIGHT)
    {
        ...
        const unsigned long timestamp = std::chrono::duration_cast<std::chrono::nanoseconds>(now.time_since_epoch()).count();
